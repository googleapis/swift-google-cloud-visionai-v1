...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-visionai-v1.git --from 0.0.0-preview
```

Then add `GoogleCloudVideoAIV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudVideoAIV1 <target-name> --package swift-google-cloud-visionai-v1
```
